...
export(Dictionary) var _states := {} # vfsm_id to VFSMState
export(Dictionary) var _trigger_vfsm_id_map := {} # vfsm_id to VFSMTrigger
export(Dictionary) var _transitions := {
...
func remove_trigger_from_state(state: VFSMState, trigger: VFSMTrigger) -> void:
	...
	var to_delete = [];
	for triggerid in _trigger_vfsm_id_map.keys():
		var delete = true;
		for state_vfsm_id in _states:
			if(_states.get(state_vfsm_id).has_trigger(triggerid)):
				delete = false;
				break;
		if(delete): to_delete.append(triggerid);
	
	for triggerid in to_delete:
		remove_trigger(_trigger_vfsm_id_map[triggerid]);
